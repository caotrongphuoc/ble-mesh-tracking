#!/usr/bin/env bash
# Produce a UF2 file that is flashable on XIAO nRF52840 (Linux/macOS).
#
...
OUT_UF2="${PROJ}/tag_Xiao_SIGNED.uf2"
...
# Fallback to the pre-rename domain name.
...
echo ""
echo "Flash: double-tap the RESET button to enter the bootloader, then copy the file above onto the XIAO-SENSE drive (mounts under /media/\$USER/ on Linux, /Volumes/ on macOS)."
